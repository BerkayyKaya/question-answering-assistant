...
st.set_page_config("Büyük Dil Modeli Demo", page_icon="https://img.icons8.com/?size=100&id=pnUfrhjBRScY&format=png&color=000000")

# Bazı yolların tanımlanması
...
# CHAT HİSTORYDEN ÖNEMLİ OLAN ŞEY ŞU ODA BİLGİSİ, SAAT BİLGİSİ TARİH BİLGİSİ HİÇBİR ŞEY YOKSA NE YAPACAK BUNU BİR ELE ALMALISIN
# ÖRNEĞİN OTURMA ODASI İÇİN EN SICAK GÜN HANGİ GÜNDÜ DEDİĞİNDE TARİH, SAAT VE SICAKLIK BİLGİSİNİN EN ÇOK OLDUĞU GÜNÜ BULUP VERMELİ
# CHAT HİSTORY EKLENEBİLİR.

# Fonskiyonlar ve Tanımlamalar

# Vektör Veritabanının Başlatılmasını Sağlayan Fonksiyon:
@st.cache_resource
...
# Kullanıcının dilini anlayacak fonksiyon:
def create_language_prompt(user_query : str):
    system_prompt = f"""
    Your goal is the detect texts main language.
    Make sure only return the detected language that you found.
    ...
    Example:
    Input: Oturma odası kaç derece?
    Output: 'tr'

    Detect the following texts language and return the detected language
    Input: {user_query}
    Output: 
    """
...
# Kullanıcı sorusunu sistem diline çevirecek fonksiyon (AI Promptları İngilizce Verildiyse İngilizceye Çevirecek):
def translate_query(user_query : str):
    system_prompt = f"""
    Translate the following text into understandable English, taking into account the rules.
    ## Rules
    1. Do not confuse theese words 'salon' and 'oturma odası' theese words means 'living room' in English.
    2. Translate the text into basic English.
    3. Make sure you only return translated text that you made.
    Text:
    {user_query}
    """
...
# Kullanıcı sorusunun nasıl cevaplanması gerektiğini belirleyen yönlendirici fonksiyon:
def route_model(user_query : str):
    ...
    messages = [
        SystemMessage(content=route_msg_template)
    ]

    return messages

# Vektör Veritabanından Belgeleri Getirmek İçin Kullanılan Fonksiyon:
def fetch_documents(structured_query : str, vectorstore, metadatas):
    if metadatas['Hour'] == "all":
        retriever = vectorstore.as_retriever(search_tpye="mmr", search_kwargs={"k":50, "fetch_k": 300})
...
# LLM'i Başlatmaya Yarayan Fonksiyon:
def initialize_llm(llm_name="gemini-2.5-flash", llm_lib="google-genai"):
    if llm_lib.lower() == "google-genai":
        llm = GoogleGenerativeAI(model=llm_name, temperature = 0.3)
    else:
        print(f"Geçersiz LLM Kütüphanesi Girişi: {llm_lib}")

    return llm

# Kullanıcı Sorusunu İşlemek İçin Prompt Hazırlayan Fonksiyon:
def create_query_prompt(user_query : str):
    ...
    HumanMessage(content="Input: Which day was the hottest for the living room?"),
    AIMessage(content='{"date": "last week", "time": "all", "room": "living room", "query": "temperature"}'),
    HumanMessage(content="Input: Which day was the hottest day?"),
    AIMessage(content='{"date": "last week", "time": "all", "room": "not mentioned", "query": "temperature"}'),
    HumanMessage(content="Input: Which day was the hottest day in kitchen?"),
    AIMessage(content='{"date": "last week", "time": "all", "room": "kitchen", "query": "temperature"}'),
    HumanMessage(content="Input: today temperature"),
    AIMessage(content='{"date": "current", "time": "not mentioned", "room": "not mentioned", "query": "temperature"}'),
    HumanMessage(content="Input: on 15 August 5 PM temperature"),
    AIMessage(content='{"date": "2025-08-15", "time": "17:00", "room": "not mentioned", "query": "temperature"}'),
    HumanMessage(content="Input: living room on 14 august 2 PM"),
    AIMessage(content='{"date": "2025-08-14", "time": "14:00", "room": "living room", "query": "not mentioned"}'),
    HumanMessage(content="Input: on 21 august 8 am living room"),
    AIMessage(content='{"date": "2025-08-21", "time": "08:00", "room": "living room", "query": "not mentioned"}')
    ]
...
# Kullanıcının Sorusunu Yeniden Düzenleyen ve Sorudaki Önemli Kısımları Çıkartan Fonksiyon:
def rewrite_query_and_extract_metadata(structured_model_output : str, user_query):
    """
    ...
    match = re.search(r'\{.*\}', structured_model_output, re.DOTALL)
    if not match:
        print(f"Girdide geçerli bir JSON yapısı bulunamadı: {structured_model_output}")
...
def create_answering_prompt(user_query: str, model_decision : str, language : str, status=None, docs_from_faiss=None, metadata_info=None):
    if "general_knowledge" in model_decision:
        ...
        Your system language is English, but you must answer the user in their own language. 
        ...
        Make sure your final answer is in users language.
        ...
        messages = [
            SystemMessage(content=answering_prompt_template)
        ]
    else:
        if ((status) and (status == "clarification_needed")):
            ...
            ### Rules
            1. Your system language is English, but you must answer the user in their own language. 
            ...
            answering_prompt_final = answering_prompt_template.format(
                ...
                language = language,
                user_language_placeholder = language
            )
            ...
            answering_prompt_template = """
                ...
                5. Your system language is English, but you must answer the user in their own language. 
                ...
                ## Example 4:
                [User Question]
                Yatak odasının şu anki sıcaklığı kaç derece?

                [Provided Information]
                Date: 2025-08-16, Time: 09:00, Room: Bedroom, Sensor: Temperature Sensor, Sensor Value: 18.4, Sensor Unit: °C, Status: Normal

                [Metadata]
                {{"Date": 2025-08-16, "Hour": 09:00, "Room": Bedroom, "Sensor": "Temperature Sensor"}}

                [User Language]
                'tr'

                [Your Answer]
                Yatak odasının 2025-08-16 tarihinde saat 09:00 daki sıcaklığı 18.4 °C olarak ölçülmüştür. Bilgi: Genellikle insanlar için ideal yatak odası sıcaklığı 16 ila 19 °C arasında olmalıdır.
                ...
                ## Now, answer the user's actual question based on the provided information, metadata information and user language:
                ## IMPORTANT: You must provide the final answer in {language}.
...
if "llm_initialized" not in st.session_state:
    ...
    st.session_state.llm_vectorstore = initialize_vectorstore()
    info.text("Vektör Veritabanı Hazır!")
    time.sleep(1)

    # Decoder modelinin hazırlanması
    info.text("Decoder Hazırlanıyor...")
    st.session_state.llm_model = initialize_llm(llm_name=decoder_name, llm_lib="google-genai")
...
if prompt := st.chat_input("Bir Şeyler Sorun"):
    ...
    with st.chat_message("assistant"):
        with st.spinner("Düşünüyor...", show_time=True):
            ...
            detect_language_prompt = create_language_prompt(user_query=prompt)

            detected_language = st.session_state.llm_model.invoke(detect_language_prompt)

            if not 'en' in detected_language:
                translate_query_prompt = translate_query(prompt)
                translated_query = st.session_state.llm_model.invoke(translate_query_prompt)
            else:
                translated_query = prompt

            route_prompt = route_model(user_query=translated_query)

            model_decision = st.session_state.llm_model.invoke(route_prompt)

            if "general_knowledge" in model_decision:
                answering_prompt_template = create_answering_prompt(user_query=translated_query, model_decision=model_decision, language=detected_language)

                model_response = st.session_state.llm_model.invoke(answering_prompt_template)
            else:
                query_prompt_template = create_query_prompt(translated_query)

                model_output = st.session_state.llm_model.invoke(query_prompt_template)

                structured_query, metadatas, status = rewrite_query_and_extract_metadata(model_output, user_query=translated_query)

                if not (status == "clarification_needed"):
                    docs_from_faiss = fetch_documents(structured_query=structured_query, vectorstore=st.session_state.llm_vectorstore, metadatas=metadatas)
                else:
                    docs_from_faiss = []

                answering_prompt_template = create_answering_prompt(translated_query, model_decision=model_decision, language=detected_language, status=status, docs_from_faiss=docs_from_faiss, metadata_info=metadatas)
